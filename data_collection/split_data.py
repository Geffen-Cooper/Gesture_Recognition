...
'''
This merges and renames all the data so the file structure is
consistent with the csvs folder (sample_i_label_c.csv)
'''
def merge_folders(data_dir):

    classes = os.listdir(data_dir)
    classes.sort(key=lambda f: int(f.split("_")[1]))

    os.mkdir(os.path.join(data_dir,os.path.basename(data_dir)))
    ...
    for c in classes:
        ...
        for file in files:
            new_name = "sample_"+str(i)+"_label_"+c.split('_')[1]+".csv"
            shutil.move(os.path.join(data_dir,c,file),os.path.join(data_dir,os.path.basename(data_dir),new_name))
            i += 1

if __name__ == '__main__':
    split_dataset("data",0.1)
    merge_folders("data/train")
    merge_folders("data/test")